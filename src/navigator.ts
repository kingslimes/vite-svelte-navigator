...
export function createBrowserRouter( routes: BrowserRouter[] ): Router {
    ...
    function navigate( to: string, options: { state?: AnyObject, replace?: boolean, clean?: boolean } = { state: {}, replace: false, clean: false } ) {
        ...
        hash.set( url.hash );
        if ( options.clean ) console.clear();
    ...
    return {
        routes, currentTarget, navigate, website: { pathname, search, state, hash },
